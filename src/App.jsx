...
function InkBg({ phase }) {
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      ctx.fillStyle = '#0e0b14'; // 거의 검정에 가까운 다크 배경
    ...
    function makeParticle(x, y, dirX, dirY, forceAlpha) {
      ...
      return {
        ...
        alpha: forceAlpha ?? (0.035 + Math.random() * 0.040), // 초기 투명도
    ...
    function loop(timestamp) {
      ...
      // 2초마다 화면 어딘가에 아주 희미한 주변 파티클 자동 생성
      if (timestamp - lastAmbient > 2000) {
        particles.push(makeParticle(
          Math.random() * W, Math.random() * H,
          (Math.random() - 0.5) * 2, (Math.random() - 0.5) * 2,
          0.008 + Math.random() * 0.010, // 아주 낮은 투명도
        ));
...
function AnalysisCard({ title, visible, style, children }) {
  return (
    ...
    >
      <p className="card-title" style={{ fontSize: '20px', fontWeight: 600 }}>{title}</p>
      {children}
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <>
      ...
      {imageUrl && (vis('uploaded') || vis('scanning')) && (
        <div style={{
          ...
          <img
            ...
            alt="memory fragment"
            style={{ width: '600px', height: '600px', objectFit: 'cover', borderRadius: '18px', display: 'block' }}
          />
          ...
          {vis('scanning') && (
            <>
              <AnalysisCard title="Color" visible={cardsIn[0]} style={{ top: '60px', left: '-80px', transform: 'rotate(-2deg)' }}>
                ...
                <p className="card-sub" style={{ fontSize: '16px' }}>dominant palette</p>
              </AnalysisCard>
              ...
              <AnalysisCard title="Blur Density" visible={cardsIn[1]} style={{ bottom: '-30px', left: '55%', transform: 'rotate(1.5deg)' }}>
                ...
                <p className="card-sub" style={{ fontSize: '16px' }}>soft diffusion</p>
              </AnalysisCard>
              ...
              <AnalysisCard title="Light Origin" visible={cardsIn[2]} style={{ top: '-20px', right: '60px', transform: 'rotate(2deg)' }}>
                ...
                <p className="card-sub" style={{ fontSize: '16px' }}>brightest region</p>
              </AnalysisCard>
              ...
              <AnalysisCard title="Motion Trace" visible={cardsIn[3]} style={{ top: '45%', right: '-90px', transform: 'rotate(-1deg)' }}>
                ...
                <p className="card-sub" style={{ fontSize: '16px' }}>directional drift</p>
              </AnalysisCard>
            </>
          )}
        </div>
      ...
      >
        <p style={{ fontSize: '24px', color: 'rgba(255,255,255,0.3)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '40px' }}>
          extraction complete
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          {/* Card 1 — Color Palette */}
          <div className={`extracted-card${extracting ? ' in' : ''}`} style={{ minWidth: '400px', padding: '24px', background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '16px', transitionDelay: '0ms' }}>
            <p style={{ fontSize: '16px', color: 'rgba(255, 255, 255, 0.35)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '16px' }}>Color Palette</p>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
              {(rules?.palette || []).map((hex, i) => (
                <span key={i} style={{ width: '56px', height: '56px', borderRadius: '50%', background: hex, display: 'inline-block' }} />
              ))}
            </div>
          </div>

          {/* Card 2 — Blur Density */}
          <div className={`extracted-card${extracting ? ' in' : ''}`} style={{ minWidth: '400px', padding: '24px', background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '16px', transitionDelay: '120ms' }}>
            <p style={{ fontSize: '16px', color: 'rgba(255,255,255,0.35)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '16px' }}>Blur Density</p>
            <p style={{ fontSize: '48px', fontWeight: 200, color: 'rgba(255,255,255,0.85)', lineHeight: 1, marginBottom: '12px' }}>
          ...
          {/* Card 3 — Light Origin */}
          <div className={`extracted-card${extracting ? ' in' : ''}`} style={{ minWidth: '400px', padding: '24px', background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '16px', transitionDelay: '240ms' }}>
            <p style={{ fontSize: '16px', color: 'rgba(255,255,255,0.35)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '16px' }}>Light Origin</p>
            <p style={{ fontSize: '48px', fontWeight: 200, color: 'rgba(255,255,255,0.85)', lineHeight: 1 }}>
              {rules ? `${Math.round(rules.lightOrigin.x * 100)} / ${Math.round(rules.lightOrigin.y * 100)}` : '—'}
            </p>
          </div>

          {/* Card 4 — Motion Trace */}
          <div className={`extracted-card${extracting ? ' in' : ''}`} style={{ minWidth: '400px', padding: '24px', background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '16px', transitionDelay: '360ms' }}>
            <p style={{ fontSize: '16px', color: 'rgba(255,255,255,0.35)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '16px' }}>Motion Trace</p>
            <p style={{ fontSize: '40px', fontWeight: 200, color: 'rgba(255,255,255,0.85)', marginBottom: '8px' }}>
      ...
      >
        ...
        <div className="compare-row" style={{ gap: '20px' }}>
          <div className="compare-card" style={{ padding: '12px', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }}>
            <span className="compare-label" style={{ fontSize: '20px' }}>BEFORE</span>
            {imageUrl && (
              <img
                src={imageUrl}
          ...
          <span className="compare-arrow" style={{ fontSize: '32px' }}>→</span>

          <div className="compare-card" style={{ padding: '12px', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }}>
            <span className="compare-label" style={{ fontSize: '20px' }}>AFTER</span>
        ...
        <ol className="step-list">
          {FLOW_STEPS.map((step, i) => (
            <li key={step} className={i === activeStage ? 'step-active' : ''}>
              <span style={{ fontSize: '20px', letterSpacing: '0.08em' }}>
